	...
	for point in pointall_A[1:]:
		if point.exi==1 :
			plt.plot([point.xy[0],pointall_A[point.dad].xy[0]],
					 ...
					 color=str((gen-point.gen*dt_A/dt)/gen))
...
def drawpoints(pointall_t,time):
	
	plt.close()
	plt.ion()
	i=0
	for pointall in pointall_t:
		i+=1
		if i%5==0 :
	...
	while success_B==0:
		...
		if point_B_num<=2000 :
			...
			for i in range(point_B_all[1].gen-1,0-1,-1):
				...
				geo_point.xy=sta_b+np.array([v*dt*i*math.cos(geo_point.direction),v*dt*i*math.sin(geo_point.direction)])
...
def getnextboundarypoints(pointall,path_B):
	...
	dy=4
	dradian_A=radian_max_A/(-area[0][1]/len(pointall)/dy+10)

	
	y_max=area[0][1]
	y_min=0
	for point in pointall:
		if random.uniform(0,len(pointall))<300 and random.uniform(0,len(pointall))<300 and random.uniform(0,len(pointall))<300:
			for directions in np.arange(point.direction-radian_max_A,point.direction+radian_max_A,dradian_A):
				...
				if obsirc(newpoint.xy) and inarea(newpoint.xy,1) :
					count=0
					while dismeet(newpoint.xy,path_B[max(int(newpoint.gen*dt_A/dt),len(path_B)-1)].xy)==0 and count<1000:
	...
	print(len(pointall),len(nextboundarypoints_may))
	
	for point_may in nextboundarypoints_may:
		
		
		if random.uniform(0,len(nextboundarypoints_may))<1000:
			if point_may.exi==1:
				
				less=0
				for point in pointall[min(point_may.dad-1+10000,0):point_may.dad]:
					
					if math.sqrt(sum((point_may.xy-point.xy)*(point_may.xy-point.xy)))<=v*dt_A:
						less=1
				for point in pointall[point_may.dad+1:max(point_may.dad+2-1000000,len(pointall))]:
				...
				if less==0:
					...
					point_may.sel=len(nextboundarypoints)
					
					pointall[point_may.dad].son.append(point_may.sel)
					nextboundarypoints.append(point_may)
			else:
...
while 1:
	...
	path_B,startgen,endgen=getpath_B()
	print(len(path_B))
	draw([],path_B,1)
	
	path_A=getpath_A(path_B,startgen,endgen)
	draw(path_B,path_A,10)
