...
def f1():
	...
	geo_initime_max=math.sqrt(sum(sta_b*sta_b)-R*R)/v_B
	...
	B0=sta_b+np.array([geo_initime_min*v_B*math.cos(directionB),geo_initime_min*v_B*math.sin(directionB)])
	...
	for t in np.arange(geo_initime_min+dt,geo_initime_max-dt,dt):
		...
		B=B0+np.array([t1*v_B*math.cos(directionB),t1*v_B*math.sin(directionB)])
		if dismeet(A,B)==0:
			return 1
	return 0
...
def f2():
	...
	geo_initime_min=0
	geo_initime_max=math.sqrt(sum(sta_a*sta_a)-R*R)/v
	directionB=math.asin(R/math.sqrt(sum(sta_a*sta_a)))-math.pi

	directionA=-math.asin(R/math.sqrt(sum(sta_a*sta_a)))
	...
	A0=sta_a
	B0=sta_a
	te=math.sqrt(sum(sta_a*sta_a)-R*R)/v_B+math.sqrt(sum(sta_b*sta_b)-R*R)/v_B+R*math.asin(R/math.sqrt(sum(sta_a*sta_a)))/v_B+R*math.asin(R/math.sqrt(sum(sta_b*sta_b)))/v_B
	for t in np.arange(geo_initime_min+dt,geo_initime_max-dt,dt):
		t1=t-geo_initime_min
		A=A0+np.array([t1*v*math.cos(directionA),t1*v*math.sin(directionA)])
		B=B0-np.array([(te-t)*v_B*math.cos(directionB),(te-t)*v_B*math.sin(directionB)])
		if dismeet(A,B)==0:
			return 1
	return 0
...
prdata2_x=[]
prdata2_y=[]
for x_te in np.arange(1000,8100,10):

	print(x_te,end=" ")
	sta_b[0]=x_te
	v_te=9
	ok=0
	while ok==0 and v_te<50:
		v_te+=0.5
	...
	if v_te>10 and v_te<50:
		
		prdata1_x.append(x_te)
		prdata1_y.append(v_te)
	print(v_te,end=" ")
	...
	if v_te>10 and v_te<50:
		prdata2_x.append(x_te)
		prdata2_y.append(v_te)
	print(v_te)




plt.plot(prdata1_x,prdata1_y,color ="red")
plt.plot(prdata2_x,prdata2_y,color ="blue")
plt.show()