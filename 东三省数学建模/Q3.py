...
def getgeo_inista_A(geo_initime):
	if sta_b[0]==1000:
		geo_initime=math.sqrt(sum(sta_a*sta_a)-R*R)/v
	geo_inista_A=Point()
...
def getpath_B():
	...
	geo_initime_max=(math.sqrt(sum(sta_b*sta_b))-R)/v_B
		...
		if error ==0:
			...
			if error ==0:
				...
				if sta_b[0]!=1000:
					...
					inf=path+os.sep+str(maxnumber)+"_"+ str(path_A[-1].gen*dt_A)+"_"+ str(path_B[-1].gen*dt)+"_" +str(-sta_a[0]) +"_"+ str(v_B)
