...
%{
obstacles=[80 350 200;50 200 10;40 200 300;60 100 150];%定义障碍：r Oy Ox
area=[500 500];%定义map大小：高，宽

%↓不用了，注释掉
%map=int16(im2bw(imread('map1.bmp'))); % input map read from a bmp file. for new maps write the file name here
source=[20 10]; % source position in Y, X format
goal=[400 400]; % goal position in Y, X format
%}

[obstacles,source,goal,area]=Inputinfo();%弹窗输入信息
map=mapCreater(obstacles,area);%生成map
...
robotHalfDiagonalDistance=((robotSize(1)/2)^2+(robotSize(2)/2)^2)^0.5; % used for distance calculations 什么意思：字面意思
...
while ~pathFound
    ...
    % calculate distance from obstacle at front 向前发出一条射线直到遇阻
    i=robotSize(1)/2+1;
    while true
    ...
    % calculate distance from obstacle at left 与上处同理
    i=robotSize(2)/2+1;
    while true
